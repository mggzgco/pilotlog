...
export function AircraftDocumentViewer({
  docId,
  filename,
  triggerLabel = "View"
}: {
        ...
        <Dialog.Content className="fixed inset-4 z-[2010] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl dark:border-slate-800 dark:bg-slate-950">
          <div className="flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-3 dark:border-slate-800">
            <div className="min-w-0">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">In-app viewer</p>
            </div>
            <div className="flex items-center gap-2">
              <Button asChild variant="ghost" size="sm">
                <a href={`/api/aircraft/documents/${docId}/download`}>Download</a>
              </Button>
          ...
          <iframe
            title={filename}
            src={`/api/aircraft/documents/${docId}/view`}
